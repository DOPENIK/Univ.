...
int main()
{
	...
	*/
	...
	for (int b = 320; b > 160; b--) {
		for (int i = 0; i < W; i++) {
			wt = i / (double)(W - 1);
			Image[b * W * 3 + i * 3] = (BYTE)(255 * wt);  // Blue
			Image[b * W * 3 + i * 3 + 1] = (BYTE)(255 * (1.0 - wt)); // Green
			Image[b * W * 3 + i * 3 + 2] = (BYTE)(255 * wt); // Red
		}
	}
	...
	for (int c = 160; c > 0; c--) {
		for (int i = 0; i < W; i++) {
			wt = i / (double)(W - 1);
			Image[c * W * 3 + i * 3] = (BYTE)(255 * (1.0 - wt));  // Blue
			Image[c * W * 3 + i * 3 + 1] = (BYTE)(255 * wt); // Green
			Image[c * W * 3 + i * 3 + 2] = (BYTE)(255 * wt); // Red
		}
	}
