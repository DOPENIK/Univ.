...
void m_BlobColoring(BYTE* CutImage, int height, int width)
{
	...
	for (k = 0; k < width * height; k++)
	{
		...
		if (BlobArea[coloring[k]] > 500) CutImage[k] = 0; // 면적이 일정 크기 이상인 것만 저장
		//CutImage[k] = (unsigned char)(coloring[k] * grayGap); 
		//원래 라벨링의 결과를 본다, 각 component를 비율에 맞춰 색을 칠한다.
	}
...
void Scaling(BYTE* Image, BYTE* Output, int W, int H, double SF_X, double SF_Y) {
	...
	for (int i = 0; i < H; i++) {
		for (int j = 0; j < W; j++) {
			//tmpX = (int)(j * SF_X);// 순방향사상
			//tmpY = (int)(i * SF_Y);// 순방향사상
			tmpX = (int)(j / SF_X);// 역방향 사상
			tmpY = (int)(i / SF_Y);// 역방향 사상
			if (tmpY < H && tmpX < W)
				//Output[tmpY * W + tmpX] = Image[i * W + j]; // 순방향사상
...
void Rotation(BYTE* Image, BYTE* Output, int W, int H, int Angle) {
	...
	for (int i = 0; i < H; i++) {
		for (int j = 0; j < W; j++) {
			tmpX = (int)(cos(Radian)*j-sin(Radian)*i); // 순방향 사상
			tmpY = (int)(sin(Radian)*j+cos(Radian)*i); // 순방향 사상
			//tmpX = (int)(cos(Radian) * j + sin(Radian) * i); // 역방향 사상, 회전행렬의 역행렬
			//tmpY = (int)(-sin(Radian) * j + cos(Radian) * i); // 역방향 사상, 회전행렬의 역행렬
			if ((tmpY < H && tmpY >= 0) && (tmpX < W && tmpX >= 0))
				Output[tmpY*W+tmpX] = Image[i*W+j]; //순방향 사상, cos,sin 쓰니까 소수점 없어지면서 hole 생김
...
int main()
{
	...
	//Scaling(Image, Output, W, H, 0.7, 0.7); // x_scaling_vector와 y_scaling_vector가 같으면 Uniform scaling이다.
	...
	InverseImage(Temp, Temp, W, H); // 흑과 백을 뒤집어서 동공이 검출되도록
	m_BlobColoring(Temp, H, W); // Labeling
	...
	DrawRectOutline(Temp,Output, W, H, LU_X, LU_Y, RD_X, RD_Y);*/ //동공 라인따기 과제

	

	//AverageConv(Image, Output, hInfo.biWidth, hInfo.biHeight); // 평활화를 해준다.

	SaveBMPFile(hf, hInfo, hRGB, Output, hInfo.biWidth, hInfo.biHeight, "output.bmp"); // 파일 저장하는 것을 별도의 함수로 짧게 표현했다.
